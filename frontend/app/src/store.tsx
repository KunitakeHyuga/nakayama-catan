      ...
      switch (action.type) {
        ...
        case ACTIONS.SET_GAME_STATE: {
          ...
          const newActionRecords =
            prevGameState && nextGameState && prevActionCount !== null
              ? nextGameState.action_records.slice(prevActionCount)
              : [];
          const newRoadBuilds =
            state.isRoadBuilding && newActionRecords.length > 0
              ? newActionRecords.filter(
                  ([action]) => action[1] === "BUILD_ROAD"
                ).length
              : 0;
          const nextFreeRoadsAvailable = state.isRoadBuilding
            ? Math.max(state.freeRoadsAvailable - newRoadBuilds, 0)
            : 0;
          return {
            ...state,
            ...
            isBuildingSettlement: false,
            isBuildingCity: false,
            isRoadBuilding: nextFreeRoadsAvailable > 0,
            freeRoadsAvailable: nextFreeRoadsAvailable,
            isPlayingMonopoly: false,
            isPlayingYearOfPlenty: false,
            isMovingRobber: false,
          };
